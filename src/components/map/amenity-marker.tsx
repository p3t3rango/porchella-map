...
export function AmenityMarker({ type, label }: AmenityMarkerProps) {
  const Icon = icons[type];
  return (
    <div
      role="img"
      aria-label={label}
      className={`group relative flex h-7 w-7 items-center justify-center rounded-md border-2 shadow-sm ${colors[type]}`}
    >
